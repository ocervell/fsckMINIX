...
int (* const call_vec[NR_VFS_CALLS])(void) = {
	...
	CALL(VFS_INODEWALKER)   = do_inodewalker,	/* inodewalker(2) */
	CALL(VFS_ZONEWALKER)	= do_zonewalker,	/* zonewalker(2) */
/*	CALL(VFS_RECOVERY)		= do_recovery,		 recovery(2)
	CALL(VFS_DAMAGEINODE)   = do_damage_inode,	 damageinode(2)
	CALL(VFS_DAMAGEDIR)		= do_damage_dir,		 damagedir(2) */
};
